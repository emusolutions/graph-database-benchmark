...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-name', type=str, default="k-hop-no-params", help='the name of the test')
    ...
    parser.add_argument('--benchmark-output-file-prefix', type=str, default="k-hop-no-params.redisgraph.commands",
                        help='prefix to be used when generating the artifacts')
    parser.add_argument('--benchmark-config-file', type=str, default="k-hop-no-params.redisgraph.cfg.json",
    ...
    for query_number in tqdm(range(1, args.total_benchmark_commands + 1)):
        ...
        cypher = "MATCH (n)-[*{k_hop}]->(m) WHERE id(n)={seed} RETURN count(distinct m);".format(
            seed=seed, k_hop=k_hop)
        cmd = ["READ", "{}-HOP-NO-PARAMS".format(k_hop), "GRAPH.QUERY", graph_key_name, cypher]
        queries.append(cmd)
