...
# IS5 query
IS5_query = "MATCH (m:Message {{id: {messageId} }})-[:HAS_CREATOR]->(p:Person) RETURN p.id AS personId, p.firstName AS firstName, p.lastName AS lastName"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
            "--comment-file", type=str, default='comment_0_0.csv', help="ldbc comment file"
        )
    ...
    parser.add_argument('--test-name', type=str, default="ldbc-is5-no-params", help='the name of the test')
    ...
    parser.add_argument('--benchmark-output-file-prefix', type=str, default="ldbc-is5-no-params.redisgraph.commands",
                        help='prefix to be used when generating the artifacts')
    parser.add_argument('--benchmark-config-file', type=str, default="lsbc-is5-no-params.redisgraph.cfg.json",
    ...
    ids = []
    setup_queries = []
    print("Reading comment file")

    with open(args.comment_file) as csvfile:
    ...
    for cmd_number in tqdm(range(1,args.total_benchmark_commands+1)):
        message_id = ids[total_ids%cmd_number-1]
        q = IS5_query.format(messageId=message_id)
        cmd = ["READ", "IS5", "GRAPH.QUERY", graph_key_name, q]
